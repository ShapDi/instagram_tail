...
class Tail:
    ...
    async def get_data_of_user(self, target_username: str, host_account: Account, pool: AccountPool) -> CollectedData:
        ...
        try:
            ...
            return CollectedData(account=ParsingError(f"Ошибка прокси: {proxy.url}, ошибка: {e}"), posts=None)
        except httpx.ConnectError as e:
            print(f"Ошибка подключения: {e}")
            return CollectedData(account=ParsingError(f"Ошибка подключения: {proxy.url}, ошибка: {e}"), posts=None)
        except httpx.ReadTimeout as e:
            print("Таймаут при чтении ответа")
            return CollectedData(account=ParsingError(f"Таймаут при чтении ответа: {proxy.url}, ошибка: {e}"), posts=None)
            ...
            return CollectedData(account=ParsingError(f"Ошибка при входе: {e}"), posts=None)
