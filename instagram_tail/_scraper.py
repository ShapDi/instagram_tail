...
class Scraper:
    async def get_account_data(
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                print(
                    f"Статус 302 при парсинге {username}. Текст: {e.response.headers}"
                )
                redirect_url = urljoin(url, r.headers.get("location", ""))
                if "/challenge/" in redirect_url:
                    raise AccountBlockedException(
                        f"Аккаунт требует прохождения капчи. Редирект: {redirect_url}"
                    )
                return ParsingError(
                    f"При парсинге аккаунта {username} произошёл редирект (возможно аккаунт, с которого происходит парсинг заблокирован)"
                )
            return ParsingError(
                f"Ошибка при запросе страницы аккаунта {username}: {e.response.status_code}"
            )
        ...
        if match:
            ...
            return ParsingError(
                f"Не удалось извлечь user ID для {username}. Возможна блокировка или изменения в разметке."
            )
        ...
        try:
            ...
            return ParsingError(
                "Ошибка при декодировании JSON-ответа об аккаунте от GraphQL."
            )
        ...
        if not user:
            return ParsingError(
                f"Нет данных о пользователе с id {self.id}. Аккаунт может быть заблокирован."
            )
    ...
    async def get_all_posts(
        ...
        while has_next_page:
            ...
            try:
                ...
                return [
                    ParsingError(
                        f"Ошибка при запросе или декодировании JSON при сборе постов: {e}"
                    )
                ]
    ...
    async def get_post_info(
        ...
        try:
            ...
            return ParsingError(
                f"Ошибка запроса или декодирования JSON для поста {post_id}: {e}"
            )
        ...
        if media is None:
            return ParsingError(
                f"Пост {post_id} недоступен (возможно, возрастное ограничение или геоблок)"
            )
